...
const Hero: React.FC = () => {
  return (
    <section className="relative overflow-hidden bg-[#f9f6f1] manrope-para">
      <div className="absolute top-0 left-0 w-full md:w-[75%] h-[80%] bg-[#C19355] rounded-br-[180px]" />
      <div className="max-w-6xl mx-auto relative z-10 px-6 lg:px-8 pt-10 pb-20 md:pt-20 md:pb-28">
        <div className="grid gap-12 md:grid-cols-[1.15fr_0.85fr] items-center">
          <div>
            <p className="uppercase text-xs tracking-[0.6em] text-[#FFFFFF] mb-6">Interiorfirm</p>
            <h1 className="text-4xl md:text-6xl  text-[#1c1a18] leading-tight">
              Modern Interior
              <br />
              Design Service
            </h1>
            <p className="text-lg md:text-xl text-[#FFFFFF] mt-6 max-w-2xl manrope-para">
            ...
            <div className="mt-12 grid grid-cols-2 gap-10">
              {stats.map((item) => (
                <div key={item.label}>
                  <p className="text-4xl md:text-5xl  text-primary">{item.value}</p>
                  <p className="uppercase text-xs tracking-[0.4em] text-[#FFFFFF] mt-2">{item.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div className="absolute -left-8 md:-left-16 bottom-8 w-28 h-28 md:w-36 md:h-36 rounded-full border-4 border-white bg-[#C19355] shadow-xl" />
            <div className="absolute -bottom-10 right-6 bg-white rounded-3xl shadow-xl p-4 text-sm text-[#414141] max-w-[220px]">
              <p className=" text-lg text-primary">Make your dream home.</p>
